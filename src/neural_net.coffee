...
class NeuralNet
  constructor: (input, layers, outputs, options) ->
    ...
    for el, i in  all when i > 0
      @edges.push [1..all[i]].map -> [1..all[i - 1]].map -> 0
  ...
  train: (input, output) ->
    ...
    estimated = current
    derivativeA = estimate.map (x, i) -> x - output[i]
    ...
    for edge, i in @edges by -1
      ...
      derivativeZ = derivativeA.map (x) -> x * @rectifier.derivative unrectified[i]
      ...
      derivativeA = numeric.dot(edge, derivativeZ)
    ...
    #
    # We want dC/dw[i] to have dimensions (layers[i - 1] x layers[i]).
    edgeUpdates = derivatives.map (derivative, i) ->
      numeric.dot numeric.transpose[rectified[i]], [derivative]
